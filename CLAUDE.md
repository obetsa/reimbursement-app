...
**Версійність на Docker Hub** (з 07.07.2026): перед перезаписом `latest` — попередній `latest` спершу тегується наступним по порядку `vN` і пушиться окремо (архів), і тільки після цього новий код пушиться як `latest`. Поточні теги: `v1` (30.04) → `v2` (12.05) → `v3` (19.06) → `v4` (30.06) → `v5` (07.07) → `v6` (07.07) → `v7` (07.07, архів latest до фіксів корзини/мобільного дизайну) → `latest` (07.07, коміт `286d46d`, поточний код). Наступний архівний тег — `v8`. Що саме в кожному тегу — `CHANGELOG.md` в корені репо.
